...
pub fn texture_to_gl_texture(scene: &scene::Scene, texture_name: &str) -> (image::DynamicImage, (i16, i16)) {
    ...
    for texture_lump in texture_lumps {
        ...
        for this_texture_offset in texture_offsets {
            ...
            for _i in 0..patch_count {
                let originx =
                    i16::from_le_bytes([wad_file[offset + patch_index + 22], wad_file[offset + patch_index + 23]]);

                let originy =
                    i16::from_le_bytes([wad_file[offset + patch_index + 24], wad_file[offset + patch_index + 25]]);

                let patch =
                    i16::from_le_bytes([wad_file[offset + patch_index + 26], wad_file[offset + patch_index + 27]])
                        as usize;
